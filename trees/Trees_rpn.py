...
#####
# Towards lambda?
# We drop anonymous functions, as the replication is extensive 
# (hence 'actionLabel'). But single param and term-building are present.
# use Data

class Label(CodeMark):
    def __repr__(self):
        return "{}".format(self.parsedData)
        
def mkLabel(position, text):
    t = Label()
    t.parsedData = text
    t.position = position
    return t  
